...
impl TypeChecker {
    ...
    fn check_stmt(&mut self, stmt: &Stmt) {
        match stmt {
            ...
            Stmt::Constant(_) => {}
            Stmt::Expression(e) => {
                // Auto-declare variable on `ident = expr` (immutable declaration)
                if let Expr::Assignment { target, value, .. } = e {
                    if let Expr::Identifier { name, .. } = target.as_ref() {
                        if self.symbols.lookup(name).is_none() {
                            let inferred = self.infer_expr(value);
                            let sym = Symbol::new_var(name.clone(), Some(inferred), false);
                            let _ = self.symbols.insert(name.clone(), sym);
                            return;
                        }
                    }
                }
