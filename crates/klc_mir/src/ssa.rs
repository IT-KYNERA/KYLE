// klc_mir::ssa — SSA Form conversion and representation
//
...
pub fn convert_function(func: &MirFunction) -> Option<SsaFunction> {
    ...
    let non_promotable: HashSet<usize> = func.basic_blocks.iter()
        ...
        for inst in &bb.insts {
            match inst {
                MirInst::Alloca { dest, type_, .. } => {
                    allocas.insert(*dest);
                    // Move types, structs, lists, strings cannot be promoted
                    if is_move_type(type_) || matches!(type_, MirType::Struct(_, _)) {
                        escaped.insert(*dest);
                    }
                }
...
fn value_type(value: &MirValue, func: &MirFunction, promotable: &HashSet<usize>, ssa: &SsaFunction) -> MirType {
    match value {
