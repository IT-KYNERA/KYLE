use klc_core::resolver::{resolve_from_strings, RegistryBackend, ResolvedGraph};
...
impl Manifest {
    ...
    pub fn read(path: &Path) -> Result<Self, String> {
        ...
        Self::from_str(&content)
            .map_err(|e| format!("Failed to parse {}: {}\nTip: check for syntax errors in kl.toml", path.display(), e))
    }
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        if name.is_empty() {
            errors.push("project name is required (set 'name' or [project].name in kl.toml)".into());
        ...
        if version.is_empty() {
            errors.push("project version is required (set 'version' or [project].version in kl.toml)".into());
        ...
        if !edition.is_empty() && edition != "2024" {
            errors.push(format!(
                "unsupported edition '{}' (supported: 2024).\n\
                 Tip: set edition = \"2024\" in kl.toml",
                edition
            ));
        }
...
"#;
        ...
        assert_eq!(manifest.project_main(), "src/main.kl");
