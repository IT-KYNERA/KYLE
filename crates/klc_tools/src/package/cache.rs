...
const CACHE_DIR: &str = ".kl/cache";
const KL_HOME_ENV: &str = "KL_HOME";

/// Returns the cache root directory (~/.kl/cache/ or $KL_HOME/cache/).
pub fn cache_root() -> PathBuf {
    if let Ok(home) = std::env::var(KL_HOME_ENV) {
        PathBuf::from(home).join("cache")
    } else if let Ok(home) = std::env::var("HOME") {
        PathBuf::from(home).join(CACHE_DIR)
    } else {
        PathBuf::from(".kl").join("cache")
    }
}

/// Returns the path for a cached package: ~/.kl/cache/<name>-<version>/
...
pub fn package_manifest_path(name: &str, version: &str) -> PathBuf {
    package_cache_dir(name, version).join("kl.toml")
}
...
pub fn is_cached(name: &str, version: &str) -> bool {
    ...
    dir.exists() && dir.join("kl.toml").exists()
}
...
pub fn list_cached_packages() -> Result<Vec<(String, String)>, String> {
        ...
        .map_err(|e| format!("Failed to read cache dir: {}", e))?
    {
        ...
        if let Some(sep_pos) = dir_name.rfind('-') {
            ...
            if entry.path().is_dir() && entry.path().join("kl.toml").exists() {
