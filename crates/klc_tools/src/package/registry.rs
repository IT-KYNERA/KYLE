use klc_core::resolver::{DepSpec, RegistryBackend};
use klc_core::semver::{parse_version, parse_requirement, version_to_string, SemanticVersion};
...
/// Dependency entry in a package's kl.toml from registry.
#[derive(Debug, Deserialize)]
...
impl FileRegistry {
    ...
    fn read_manifest(&self, name: &str, version: &str) -> Result<crate::package::Manifest, String> {
        let path = self.package_dir(name).join(version).join("kl.toml");
...
pub fn fetch_package_manifest(name: &str, version: &str) -> Result<String, String> {
    let registry_url = std::env::var(REGISTRY_ENV).unwrap_or_else(|_| DEFAULT_REGISTRY.to_string());
    let url = format!("{}/packages/{}/{}/kl.toml", registry_url, name, version);
