...
/// Find the project root by walking up from `start` looking for kl.toml.
pub fn find_project_root(start: &Path) -> Option<PathBuf> {
    let mut current = Some(start.to_path_buf());
    while let Some(dir) = current {
        if dir.join("kl.toml").exists() {
            return Some(dir);
        }
...
pub fn main_source_path(project_root: &Path) -> Option<PathBuf> {
    let path = project_root.join("src").join("main.kl");
    ...
    if tests_dir.is_dir() {
        if let Ok(entries) = std::fs::read_dir(&tests_dir) {
            for entry in entries.flatten() {
                let path = entry.path();
                if path.extension().map_or(false, |e| e == "kl") {
                    paths.push(path);
                }
            }
        }
