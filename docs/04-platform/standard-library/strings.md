...
`ky_str_builder_append` reallocates with doubling strategy (2× capacity) when the buffer is full, achieving amortized O(1) append. Compared to `s + "x"` (which allocates + copies on every concat), str_builder is **~380× faster** for 50k concatenations.
