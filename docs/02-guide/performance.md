...
Kyle compiles to native code via LLVM 18 with the full `-O3` optimization pipeline. Performance is comparable to C and Rust for most workloads.
...
String concatenation with `+` allocates a new string. For building strings, minimize intermediate allocations:
...
|-------------|--------|--------|
| SSA + mem2reg | ✅ | Eliminates redundant allocas |
