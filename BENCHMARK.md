...
|-----------|---|---|----------|------|
| **Fib 50M** | 110ms | 110ms | **178ms** | 1.62x |
...
- **Fib**: 1.62x vs C. All add/sub/mul now have `nsw`+`nuw` flags for LLVM optimization. The remaining gap is from Kyle's `while` loop generating a 2-block structure (header + body), while C's `for` is optimized to a single-block loop. Future work: `!llvm.loop` metadata or loop rotation in MIR.
